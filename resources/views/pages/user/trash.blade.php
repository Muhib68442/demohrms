<x-app-layout>
    ...
    <x-slot name="header">
        <div class="flex items-center gap-3">
            <a href="{{ route('users.index') }}" class="p-2 rounded-md hover:bg-gray-700 transition dark:hover:bg-gray-600 dark:text-white">
        ...
        <div class="relative overflow-x-auto shadow-md sm:rounded-lg mt-4 p-4">
            ...
            @if($users->isEmpty())
                <div class="text-center py-12 text-gray-500 dark:text-gray-400">
                    <p class="text-xl">No trashed users found</p>
                </div>
            @else
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @foreach ($users as $data)
                            <tr class="bg-white dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-600 transition duration-150 ease-in-out">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 dark:text-gray-300">
                                    {{ $data->deleted_at->format('d M Y, h:i A') }}
                                </td>
                                
                                <td class="px-6 py-4 text-sm font-medium flex items-center gap-2">
                                    <form action="{{ route('users.restore', $data->id) }}" method="post" class="inline-block">
                                        @csrf
                                    ...
                                    <form action="{{ route('users.forceDelete', $data->id) }}" method="post" class="inline-block" onsubmit="return confirm('Permanently delete this employee? This cannot be undone!')">
                                        @csrf
                                        @method('DELETE')
