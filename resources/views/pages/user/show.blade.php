                ...
                <div class="p-6">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg">
                            ...
                            <p class="text-gray-900 dark:text-gray-100 ml-8">{{ $user->created_at->format('d M Y') }}</p>
                        </div>
                        ...
                        <div class="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg">
                            ...
                            <p class="text-gray-900 dark:text-gray-100 ml-8">{{ $user->updated_at->format('d M Y') }}</p>
                        </div>
                    ...
                    <div class="flex gap-3 mt-8 justify-end">
                        <form action="{{route('users.status', $user->id)}}" method="post">
                            ...
                            @method('PATCH')
                            <button type="submit" 
                                    class="inline-flex items-center gap-2 px-4 py-2 bg-{{ $user->status == 'active' ? 'red' : 'green' }}-600 text-white rounded-md hover:bg-{{ $user->status == 'active' ? 'red' : 'green' }}-700 transition">
                        ...
                        <x-secondary-button href="{{ route('users.edit', $user->id) }}">
                            {{ svg('css-pen', 'w-5 h-5') }}
                            Edit
                        </x-secondary-button>
